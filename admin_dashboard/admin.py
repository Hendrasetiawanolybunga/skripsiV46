...
from .models import Admin, Pelanggan, Produk, Transaksi, DetailTransaksi, DiskonPelanggan, Notifikasi, Kategori, Karyawan


# 🔔 MODIFIKASI: IMPORT VIEW DASHBOARD KUSTOM BARU
# Import the actual view functions from admin_dashboard.views. If unavailable,
# provide dummy fallbacks so admin.py remains import-safe during development.
try:
    from .views import custom_admin_dashboard, laporan_transaksi, laporan_produk_terlaris
except ImportError:
    ...
    custom_admin_dashboard = dummy_view
    laporan_transaksi = dummy_view
    laporan_produk_terlaris = dummy_view
...
class PelangganAdmin(BaseModelAdmin):
    ...
    def process_set_diskon(self, request, pelanggan_id):
        ...
        if is_loyal and is_ultah:
            ...
            send_notification_task.delay(
                pelanggan_id=pelanggan.id,
                tipe_pesan="Diskon Diperbarui Admin",
                isi_pesan=f"Admin telah mengatur diskon khusus {diskon.persen_diskon}% untuk Anda!",
                url_target='/produk/'
            )
    ...
    def get_urls(self):
        ...
        custom_urls = [
            ...
            # ❌ MODIFIKASI: HILANGKAN TAUTAN DASHBOARD ANALITIK DARI SIDEBAR
            # Anda menghapus baris berikut:
            # path('dashboard_analitik/', self.admin_site.admin_view(dashboard_analitik_view), name='dashboard_analitik'),
            
            # Pertahankan tautan laporan jika masih diperlukan, jika tidak, hapus juga.
            path('laporan_transaksi/', self.admin_site.admin_view(laporan_transaksi), name='laporan_transaksi'),
            path('laporan_produk_terlaris/', self.admin_site.admin_view(laporan_produk_terlaris), name='laporan_produk_terlaris'),
        ]
...
# Daftarkan model Karyawan (separate from Admin user)
@admin.register(Karyawan)
class KaryawanAdmin(BaseModelAdmin):
    list_display = ['nama', 'email', 'is_active', 'get_actions_links']
...
class KaryawanAdminForm(forms.ModelForm):
    password = forms.CharField(
        ...
        required=False,
        help_text='Masukkan password baru untuk mengubah atau isi saat membuat karyawan baru.'
    )
    ...
    def clean(self):
        ...
        # If creating new instance, password is required
        if not self.instance.pk and not pwd:
            raise forms.ValidationError('Password wajib diisi saat membuat karyawan baru melalui admin.')
        return cleaned
...
# Use the custom form in admin so passwords are hashed when created/updated via admin
KaryawanAdmin.form = KaryawanAdminForm